 ...
 */
public class BPlusTreePlanter_1<T, V extends Comparable<V>>{
    //B+树的阶
    ...
    //无参构造方法，默认阶为3
    public BPlusTreePlanter_1(){
        this(3);
    }

    //有参构造方法，可以设定B+树的阶
    public BPlusTreePlanter_1(Integer bTreeOrder){
    ...
    //
    public BPlusTreePlanter_1(Integer bTreeOrder, Node root){
        this.bTreeOrder=bTreeOrder;
        this.maxNumber = bTreeOrder + 1;
        this.left = null;
        this.root = root;
    ...
    //查询
    public List<String> find(V key){
        List<String> t = this.root.find(key);
        if(t == null){
...
//            this.right=null;
        ...
        //查找
        abstract List<String> find(V key);
        //范围查询的边界子节点定位
...
//            System.out.println("叶子节点,插入key: " + key);
            ...
            while(i<this.number){
                if(key.compareTo((V) this.keys[i]) == 0){
                    this.values.get(i).add(value_str);
                    return null;
                }else break;
            }
            //复制数组,完成添加
            Object tempKeys[] = new Object[maxNumber];
            List<List<String>> tempValues= new ArrayList<>();
            for(int len=0;len<maxNumber;len++){
...
//                System.arraycopy(tempValues, 0, this.values, 0, this.number);
                for(int t=0;t<this.number;t++){
                    List<String> test=tempValues.get(t);
...
//让拆分后的左边叶子的right指针指向拆分后的右边，右边叶子的左边指针指向左边叶子
    ...
    public String serialize(BPlusTreePlanter_1 tree){
...
//                }
    ...
    //反序列化
    public BPlusTreePlanter_1 deserialize(String data){
        ...
        BPlusTreePlanter_1<T, Integer> tree=new BPlusTreePlanter_1<>(bTreeOrder,root);
