...
public class BPlusTree <T, V extends Comparable<V>>{
    ...
    //
    public BPlusTree(Integer bTreeOrder,Node root){
        this.bTreeOrder=bTreeOrder;
        this.maxNumber = bTreeOrder + 1;
        this.left = null;
        this.root = root;
...
//                System.arraycopy(tempValues, 0, this.values, 0, this.number);
                for(int t=0;t<this.number;t++){
                    ...
                    this.values.add(tempValues.get(t));
//                    this.values.set(t,tempValues.get(t));
                }
                //有可能虽然没有节点分裂，但是实际上插入的值大于了原来的最大值，所以所有父节点的边界值都要进行更新
                Node node = this;
                while (node.parent != null){
...
//让拆分后的左边叶子的right指针指向拆分后的右边，右边叶子的左边指针指向左边叶子
    ...
    public String serialize(BPlusTree tree){
    ...
    //反序列化
    public BPlusTree deserialize(String data){
        ...
        BPlusTree<Product, Integer> tree=new BPlusTree<>(bTreeOrder,root);
