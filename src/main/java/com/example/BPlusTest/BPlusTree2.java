...
public class BPlusTree2<T, V extends Comparable<V>>{
    ...
    //
    public BPlusTree2(Integer bTreeOrder, Node root){
        this.bTreeOrder=bTreeOrder;
        this.maxNumber = bTreeOrder + 1;
        this.left = null;
        this.root = root;
...
//            System.out.println("叶子节点查找");
            ...
            for(int i=middle-1;i>=0;i--){
                if(key.compareTo((V) this.keys[i]) == 0)
                    ret.add((T)this.values[i]);
            }
            //往右检查是否有重复值
            for(int j=middle+1;j<this.number;j++){
                if(key.compareTo((V) this.keys[j]) == 0)
                    ret.add((T)this.values[j]);
            }
            ////////////-------------------/////////////////////////
            return ret;
        }
        ...
        Map<Node,Integer> rangefind(V key) {
            if(this.number <=0)
                return null;
//            System.out.println("叶子节点查找");
            Integer left = 0;
...
//让拆分后的左边叶子的right指针指向拆分后的右边，右边叶子的左边指针指向左边叶子
    ...
    public String serialize(BPlusTree2 tree){
        Node root=tree.root;
        if(root==null)return "";
    ...
    public void buildString(Node root, List<String> list) {
        ...
        if(!root.isLeaf){
            ...
            for (int i=0;i<root.number;i++){//添加叶子节点key对应的values值集合
                String json = JSON.toJSON(root.values[i]).toString();
                list.add(json);
            }
            list.add("-1");//叶子节点没有孩子，孩子数量置为-1
        }
    ...
    //反序列化
    public BPlusTree2 deserialize(String data){
        ...
        BPlusTree2<Product, Integer> tree=new BPlusTree2<>(bTreeOrder,root);
    ...
    public Node buildTree(Node parent,Queue<String> queue){
        ...
        if(isLeaf){
            for(int i=0;i<key_num;i++){
                Product product=JSON.parseObject(queue.poll(),Product.class);
                node.values[i] = product;
            }
        }
        int size = Integer.parseInt(queue.poll());//孩子数量
        for(int i=0;i<size;i++){
            node.childs[i] = buildTree(node,queue);
        }
        for(int i=0;i<size;i++){
            ...
            if(i>0){
                ...
                node.childs[i].left=left;
            }
            if(i<size-1){
                right=node.childs[i+1];
                node.childs[i].right=right;
            };
        }
